...
-   **Main Findings and Takeaways：**
    -   對於「工作場所」、「工作時間」、「薪水」、「工作負荷量」、「性別工作平等」、「主管」、「同事相處」、「員工申訴管道」、「人事考核升遷制度」、「員工教育訓練」越滿意，則對工作越滿意。年齡越高、認為將來失業的可能性越低、在工作中未遭遇到困難，亦會對工作越滿意。且影響力最大的因素為工作時間和同事相處情形。
-   **Future Direction：**
    -   大致上為最終的版本。
...
cor.mat <- dat %>% 
  ...
  mutate(
    ...
    work_income = recode(
      work_income,
      "0-26399" = 1, "26400-29999" = 2, "30000-39999" = 3, "40000-59999" = 4, "60000-79999" = 5, "80000-99999" = 6, "100000+" = 7
    ),
...
plot_overtime.1 <- plot_overtime.base +
  geom_histogram() +
  labs(title = "Overtime and Overall Satisfaction", y="Count")
plot_overtime.2 <- plot_overtime.base +
  geom_histogram(position="fill") +
  geom_vline(xintercept = 5, color="red") +
  annotate("text", x=9, y=.5, label="Q3=5", color="red") +
  xlim(0, 51) +
  labs(
    title = "Overtime and Overall Satisfaction",
    subtitle = "Zoomed at Overtime in [0, 51]",
    y = "Percentage (%)"
  )
...
```{r income-age}
plot_income_age.base <- dat.1 %>% 
  select(c(work_income, age)) %>% 
  mutate(
    # y = as.factor(y),
    # y = recode(y, "1" = "satisfied", "0" = "not satisfied"),
    # work_income = fct_relevel(
      # work_income, "0-26399", "26400-29999", "30000-39999", 
      # "40000-59999", "60000-79999", "80000-99999", "100000+"
    # ),
    work_income = fct_recode(
      work_income, "0-26"="0-26399", "26-30"="26400-29999", "30-40"="30000-39999",
      "40-60"="40000-59999", "60-80"="60000-79999", "80-100"="80000-99999", "100+"="100000+"
    )
  ) %>% 
  ggplot(aes(age, fill=work_income)) +
  labs(
    x = "Age",
    fill = "Work Income (1000 TWD)"
  ) +
  theme_light()

plot_income_age.1 <- plot_income_age.base +
  geom_bar() +
  labs(title = "Age and Work Income")
plot_income_age.2 <- plot_income_age.base +
  geom_bar(position="fill") +
  labs(title = "Age and Work Income", y="Percentage (%)")

ggarrange(plot_income_age.1, plot_income_age.2, nrow=2, common.legend = T, legend="bottom")
```

我們可以看到年齡和收入之間存在著一定的關係，當年齡越高，收入也會跟著升高。
因此我們認為儘管收入在 lasso regression 中並沒有被篩選掉，但是控制年齡後，收入的解釋力就跟著下降。


```{r treat-likert-as-binary, include=F, echo=F}
# The model does not outperformed the original model (due to its higher AIC).
# Thus, the analysis is not included.

# dat.2.bi <- dat.2 %>% 
#   mutate(
#     satisfy_environment = as.numeric(as.numeric(satisfy_environment) <= 2),
#     satisfy_work_hours = as.numeric(as.numeric(satisfy_work_hours) <= 2),
#     satisfy_wage = as.numeric(as.numeric(satisfy_wage) <= 2),
#     satisfy_work_load = as.numeric(as.numeric(satisfy_work_load) <= 2),
#     satisfy_gender_equality = as.numeric(as.numeric(satisfy_gender_equality) <= 2),
#     satisfy_supervisor_care = as.numeric(as.numeric(satisfy_supervisor_care) <= 2),
#     satisfy_colleague = as.numeric(as.numeric(satisfy_colleague) <= 2),
#     satisfy_complaint_channel = as.numeric(as.numeric(satisfy_complaint_channel) <= 2),
#     satisfy_assess_and_promote = as.numeric(as.numeric(satisfy_assess_and_promote) <= 2),
#     satisfy_education_training = as.numeric(as.numeric(satisfy_education_training) <= 2),
#     satisfy_accessible = as.numeric(as.numeric(satisfy_accessible) <= 2)
#   )
# 
# logit_2 <- glm(y ~ ., data = dat.2.bi, family = "binomial")
# logit_2.robust <- coeftest(logit, vcov = vcovHC(logit, type="HC1"))
